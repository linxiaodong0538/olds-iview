      ...
      <ListHeader>
        <ListOperations>
          <Button class="margin-right-sm" type="primary"
                  @click="$router.push(`/${prefix}/${alias}/cars/index/form`)">新增
          </Button>
        </ListOperations>
        <ListSearch>
...
<script>
  ...
  import CarModal from '../../../models/cars'
  ...
  export default {
            ...
            render: (h, params) => {
              ...
              return h('ButtonGroup', [
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.$router.push(`/${this.prefix}/${this.alias}/cars/index/form/${params.row.id}`)
                    }
                  }
                }, '编辑'),
                h('Button', {
    ...
    methods: {
      async handleSetAttr () {
        ...
        await new CarModal().addPath('actions').POST({
          body: {
            type: actionType,
