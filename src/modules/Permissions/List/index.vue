      ...
      <ListHeader>
        <ListOperations>
          <Button class="margin-right-sm" type="primary"
                  @click="$router.push(`${routePrefix}/permissions/index/form`)">新增
          </Button>
        </ListOperations>
      </ListHeader>
    </List>
...
<script>
  ...
  export default {
            ...
            render: (h, params) => {
              return h('ButtonGroup', [
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.$router.push(`${this.routePrefix}/permissions/index/form/${params.row.id}`)
                    }
                  }
                }, '编辑'),
                h('Button', {
    ...
    methods: {
      ...
      async handleDelOk () {
        await this.$store.dispatch('deletePermission', {
          id: this.del.id
        })
